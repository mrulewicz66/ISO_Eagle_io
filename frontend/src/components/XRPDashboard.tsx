...
export default function XRPDashboard() {
    ...
    // Delay chart render to avoid dimension warnings
    useEffect(() => {
                            ...
                            {chartType === 'bar' ? (
                                <ComposedChart data={zoomedDisplayData} margin={{ top: isMobile ? 2 : 5, right: isMobile ? ((showCumulative || showPriceLine) ? 24 : 4) : ((showCumulative || showPriceLine) ? 60 : 10), left: isMobile ? 16 : 40, bottom: isMobile ? 12 : 30 }}>
                                    ...
                                    {showPriceLine && !showCumulative && (
                                        <YAxis yAxisId="price" orientation="right" stroke="#F59E0B" tickFormatter={(v) => `${v.toFixed(2)}`} tick={{ fontSize: isMobile ? 7 : 10, fill: "#F59E0B" }} domain={["auto", "auto"]} width={isMobile ? 1 : 60} hide={isMobile} />
                                    )}
                                    {showPriceLine && showCumulative && (
